...
optimal_params_path = f"opt_t_final_{args.steps}"
return_bottleneck = args.return_bottleneck

# Initialize TensorBoard writer
learning_rate = args.lr_time_1
run_name = f"N{args.steps}_model_lr{learning_rate}_batch{args.main_train_batch_size}_nTrain{args.num_train}_{args.log_suffix}"
log_dir = f"/netpool/homes/connor/DiffusionModels/LD3_connor/runs_delta_optimal_timesteps/{run_name}"
model_dir = f"/netpool/homes/connor/DiffusionModels/LD3_connor/runs_delta_optimal_timesteps/models"
...
if return_bottleneck:
    delta_ltt_model = Delta_LTT_model_using_Bottleneck(steps = args.steps, mlp_dropout=args.mlp_dropout)
else:
    delta_ltt_model = Delta_LTT_model(steps = args.steps, mlp_dropout=args.mlp_dropout)
...
for i in range(args.training_rounds_v1):
    for iter, batch in enumerate(trainer.train_loader):
        ...
        optimal_timesteps_list = dis_model.convert(optimal_params)
        ...
        t_list_computed = []
        for x, optimal_timesteps in zip(x_next_list, optimal_timesteps_list):
            x_next, x_list, t_list = trainer.solver.delta_sample_simple(
                ...
                fix_last_step=True,
                optimal_timesteps = optimal_timesteps,
                **trainer.solver_extra_params,
            )
            x_next_computed.append(x_next)#This was wrong the whole time?
            x_next_list_computed.append(x_list)
            t_list_computed.append(t_list) #these are the rations between timesteps

        

        optimal_timestep_ratios = optimal_timesteps_list[:, 1:] / (optimal_timesteps_list[:, :-1])
        t_list_computed = torch.stack([torch.stack(row) for row in t_list_computed]) 

        loss = loss_fn(optimal_timestep_ratios, t_list_computed)
        loss = loss.mean()
        loss.backward()
